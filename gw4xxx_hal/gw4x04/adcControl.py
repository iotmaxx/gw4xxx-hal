...
class GW4x04ADC:
    def __init__(self, consumer:str="gw4x01_adc"):
        ...
        try:    # ignore error as it is caused by wrong initialization of cs_high, mode setting is applied as expected
            self.spi.mode = gw4x04Interfaces["SPI"]["mode"]
        except:
            pass
        self.spi.max_speed_hz = gw4x04Interfaces["SPI"]["max_speed"]
...
#        convResult /= 250
    ...
    def setOutputCurrent(self, current):
        ...
        with SMBus(gw4x04Interfaces["I2C"]["bus"]) as bus:
    #        hex_string = "".join("%02x " % b for b in data)
    #        print("write: "+hex_string)
            msg = i2c_msg.write(gw4x04Interfaces["I2C"]["MCP4176Address"], data)
            bus.i2c_rdwr(msg)
    ...
    def getOutputCurrent(self):
        with SMBus(gw4x04Interfaces["I2C"]["bus"]) as bus:
            msg = i2c_msg.read(gw4x04Interfaces["I2C"]["MCP4176Address"], 6)
            bus.i2c_rdwr(msg)
        response = list(msg)
